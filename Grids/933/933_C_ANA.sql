...
-- Create temp table for every player who played C for ANA franchise
...
-- Query every player who played C for ANA franchise
SELECT DISTINCT(ana.playerID), ana.nameFirst, ana.nameLast
FROM #ana AS ana
INNER JOIN #anac AS anac
	ON ana.playerID = anac.playerID;